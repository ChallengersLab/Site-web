...
interface BUMethodologyProps {
  methodology: BUConfig["methodology"];
  accent: string;
  badge: string;
  relatedArticleSlugs?: string[];
}

export function BUMethodology({
  methodology,
  accent,
  badge,
  relatedArticleSlugs,
          ...
          <div className="lg:sticky lg:top-32 lg:w-[360px] lg:shrink-0">
            ...
            <ScrollReveal delay={0.1}>
              ...
              {relatedArticleSlugs && relatedArticleSlugs.length > 0 && (
                <Link
                  href={`/ressources/${relatedArticleSlugs[0]}`}
                  className="group/link mt-6 inline-flex items-center gap-2 text-[13px] font-semibold transition-all"
                  style={{ color: `${accent}B3` }}
                  onMouseEnter={(e) => {
                    (e.currentTarget as HTMLElement).style.color = accent;
                  }}
                  onMouseLeave={(e) => {
                    (e.currentTarget as HTMLElement).style.color = `${accent}B3`;
                  }}
                >
