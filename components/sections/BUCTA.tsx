...
interface BUCTAProps {
  config: BUConfig;
}

export function BUCTA({ config }: BUCTAProps) {
  ...
  const gradientWord =
    config.id === "sales" ? "vos ventes" : "vos automatisations";
    ...
    <section ref={sectionRef} className="relative py-40 overflow-hidden" id="contact">
      ...
      >
        <div
          ...
          style={{
            background: `radial-gradient(circle, ${config.accent}40 0%, transparent 60%)`,
            filter: "blur(100px)",
      ...
      >
        <div
          className="h-full w-full rounded-full"
          style={{
            background: `radial-gradient(circle, ${config.accent}1E 0%, transparent 65%)`,
            filter: "blur(80px)",
      ...
      <div className="relative z-10 mx-auto max-w-[700px] px-6 text-center">
        <ScrollReveal>
          <h2 className="font-display text-[clamp(2.5rem,5.5vw,4.5rem)] leading-[0.95] tracking-[-0.02em]">
            Parlons de{" "}
            <em className="gradient-text">{gradientWord}</em>
          </h2>
          <p className="mx-auto mt-7 max-w-md text-[16px] leading-[1.7] text-white/35">
            30 minutes avec {config.director.name}. On identifie ce qu&apos;on
            peut faire pour vous — et surtout ce qu&apos;on ne fera pas.
          </p>
        </ScrollReveal>
